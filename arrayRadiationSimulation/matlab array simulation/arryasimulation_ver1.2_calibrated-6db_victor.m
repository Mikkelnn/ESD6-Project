...
% Applies calibration correction in dB and computes directivity
% ------------------------------------------
...
dtheta = deg2rad(mean(diff(theta_unique)));
dphi = deg2rad(mean(diff(phi_unique)));

% Total radiated power (∝ |E|^2 * sin(theta) integrated over sphere)
E_power = abs(E_total).^2;
Prad = sum(sum(E_power .* sin(Theta))) * dtheta * dphi;

% Corrected Gain (subtracting element factor: 10*log10(N))
gain_dB_corrected = max_gain_dB - 10 * log10(N);


%% Results
fprintf('--- Radiation Metrics ---\n');
fprintf('Max E-field amplitude: %.3f\n', max_val);
fprintf('Calibrated Gain Estimate: %.2f dB\n', max_gain_dB);
fprintf('Array Gain with N correction: %.2f dB\n', gain_dB_corrected);
