...
for i = 1:num_elements
    ...
    E_total_dB = data(:,8);                   % ETotal column in dB
E_total = 10.^(E_total_dB / 20);          % Convert to linear magnitude
...
AF_mag_dB = 20 * log10(AF_mag);  % Already calibrated what
...
for i = 1:num_elements
    data = readmatrix(antenna_files{i});
    idx = data(:,3) == frequency;
    plot(data(idx,8));  % ETotal dB
end
title('ETotal (dB) of Each Antenna'); ylabel('dB'); xlabel('Sample');
